...
def make_env_model_agent(ENV_NAME):
    ...
    dueling_type		            Duelingネットワークのタイプ（enable_dueling_networkがTrueのときに使用）。
                                    ...
                                    avgが推奨されているとのこと。具体的な式以下を参照。
    """
    from rl.agents.dqn import DQNAgent
    agent = DQNAgent(nb_actions=nb_actions, memory=memory, gamma=1.0, batch_size=32, nb_steps_warmup=1000,  # nb_steps_warmup=1000,
                    train_interval=1, memory_interval=1, target_model_update=0.9, #target_model_update=10000,
    ...
    for flg_1 in flgs:
        ...
        for flg_2 in flgs:
            ...
            Q_tmp = jis_df[(pd.to_datetime(jis_df['datetime'])>=start_date[try_no]-datetime.timedelta(hours=past)) & \
                           (pd.to_datetime(jis_df['datetime'])<=start_date[try_no]+datetime.timedelta(minutes=term*2+504*10))]
            ...
            rain_p_tmp = rain_p_df[(pd.to_datetime(rain_p_df['datetime'])>=start_date[try_no]-datetime.timedelta(hours=past)) & \
                                   (pd.to_datetime(rain_p_df['datetime'])<=start_date[try_no]+datetime.timedelta(minutes=term*2+504*10))]
        ...
        history = agent.fit(env=env, nb_steps=5000000, action_repetition=1, callbacks=None, verbose=0,
        ...
        Q_tmp = jis_df[(pd.to_datetime(jis_df['datetime'])>=start_date[kensho_no]-datetime.timedelta(hours=past)) & \
                       (pd.to_datetime(jis_df['datetime'])<=start_date[kensho_no]+datetime.timedelta(minutes=term*2+504*10))]
        ...
        rain_p_tmp = rain_p_df[(pd.to_datetime(rain_p_df['datetime'])>=start_date[kensho_no]-datetime.timedelta(hours=past)) & \
                               (pd.to_datetime(rain_p_df['datetime'])<=start_date[kensho_no]+datetime.timedelta(minutes=term*2+504*10))]

...
class dam(gym.Env):
    def __init__(self):
        ...
        #各種データのファイル名設定
        self.path_Qcon = './nomura_dam/Qcon.csv'   #開度別コンジットゲート放流テーブル!!!
        self.path_Qcre = './nomura_dam/Qcre.csv'  #開度別クレストゲート放流テーブル!!!
        self.path_gensoku = './nomura_dam/gensoku.csv'    #放流の原則テーブル!!!
        self.path_HV = './nomura_dam/HV.csv'      #HVテーブル!!!
        self.path_jis = './nomura_dam/jisseki_tmp.csv'      #実績!!!
        self.path_rain_p = './nomura_dam/rain_tmp_p.csv'      #予測雨量!!!

        self.path_res= './nomura_dam/res/res.csv'                #利水計算結果
        self.path_grhp='./nomura_dam/res/praph.png'              #利水計算結果グラフ
    ...
    def _reset(self):                                                                                    # 洪水イベントを抽出
        self.jis        = pd.read_csv(self.path_jis)
        self.rain_p     = pd.read_csv(self.path_rain_p)
        self.Qin_peak = self.jis['Qin'].max()
        self.max_steps = len(self.jis) / 2
        ...
        #self.H        = self.jis['H'].values[self.steps]
        #self.H        = self.Hs - 0.5 - random.uniform(0, 4.5)
        self.H        = self.Hs - 1.0 - (self.count % 5)
    ...
    def _step(self, action):# 初回1ステップ実行
        ...
        if self.Hs <= self.H:
            ...
            #        self.Qout = min(self.Qin, Qout_gensoku_max+self.Qout, 299.99)
                ...
                if self.Qout > 300.0:
                    ...
                    Qout_gensoku_max = self.gensoku[(self.gensoku['Qout_d']<=self.Qout) & (self.gensoku['Qout_u']>self.Qout)]['dQout'].values[0]
                    self.Qout = min(self.Qin, self.interpolation_con([[self.H, min(self.con_open+self.Vcon*10,self.con_open_max)]])[0], Qout_gensoku_max+self.Qout, 299.99)
            ...
            if self.Qout >= 300:
                ...
                Qout_list_max    = max(Qout_list_min, min(self.Qin, self.interpolation_con([[self.H, min(self.con_open+self.Vcon*10,self.con_open_max)]])[0], Qout_gensoku_max+self.Qout))
        ...
        if (self.steps>=self.max_steps-1) or ((self.Qin >= 300) and (self.Hs <= self.H)):
            self.done = 1
    ...
    def _observe(self):
        ...
        # ■DQN時の状態（初回は操作規則と同じ）
        self.obs_dqn = np.hstack([sum(np.array(self.jis['Qin'].values[self.steps-self.past:self.steps])/self.Qinmax),#ダム流入量
            sum(np.array(self.jis['r'].values[self.steps-self.past:self.steps])/self.Rmax),#流域平均雨量
            np.array([self.H_pre-self.Hs]),#ダム貯水位
            np.array([self.Qout_pre])/self.Qinmax,#ダム放流量
            ...
            np.array(self.rain_p.values[int((self.steps-self.def_rain)/18),1:])/self.Rmax])#予測雨量
            #np.array(self.jis['r'].values[self.steps+3])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+6])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+9])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+12])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+15])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+18])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+21])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+24])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+27])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+30])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+33])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+36])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+42])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+48])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+54])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+60])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+66])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+72])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+78])/self.Rmax,
            #np.array(self.jis['r'].values[self.steps+84])/self.Rmax])#予測雨量（実績）
        return self.obs_dqn
    ...
    def _get_reward(self):
        ...
        if self.rate_Qout >= 2.0:
            #self.done = 1
            reward_tmp -= pow(self.rate_Qout,2.0)/25.0
        #if self.H_pre >= self.Hs:
        ...
        if self.done == 1 and self.H < self.Hs:
            reward_tmp += 1/(self.Hs - self.H)
        elif self.done == 1 and self.H >= self.Hs and self.Qin >= 300:
            reward_tmp -= 1
        #if self.done == 1:
        #    reward_tmp += self.steps / self.max_steps
